...
回复仍有效且已映射的 Slack 通知 `bind #channel`，即可将该会话的后续消息发送到指定频道；`unbind` 恢复默认收件箱。绑定成功后，新频道会收到一条可回复的欢迎消息。权限要求与有效通知轮询说明见[会话消息目的地](docs/SESSION_DESTINATIONS.md)。每个消息提供方为每个精确会话保留四张有效回复票据；已关闭的通知仅用于审计与去重。
