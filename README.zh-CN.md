...
| macOS 15 ARM64 安装包 | **开发者预览；v0.2.3 已在指定范围内通过真实用户 E2E；v0.2.5+ 已移除 CA 临时配置要求，新版本实机 E2E 待验证** |
...
从 v0.2.8 起，分离运行的 Linux WatchDog 如果失去会话控制或监控，会通过远程主机已有的 Slack/邮件配置
发送告警；同一次故障只通知一次，确认恢复监控后再发送恢复通知。服务配置和投递状态的
检查方法见自动交接指南。
