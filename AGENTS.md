...
- **Public repository hygiene.** Remove merged public topic branches by default once their useful work is reachable from `main` and no open PR, active release or build still requires the branch. Keep GitHub's automatic head-branch deletion enabled when available; otherwise perform the same audit and deletion manually after merge. Before retiring a superseded branch, prove its work is retained, close its superseded PR, and record the old head and disposition. Preserve active release and other agents' working branches. Never rewrite history to simplify cleanup.
- **README describes the current product.** Keep the three root translations aligned and concise: purpose, architecture, supported topologies, Quick Start, current usage, and detailed-document links. Release history belongs in GitHub Releases, operational detail in `docs/`, and dogfooding chronology in `doc/Progress/`. Prepare housekeeping separately when changing a README would disturb an accepted release revision; publish the documentation-only cleanup after that release.

- **GitHub Actions authorized again (user instruction, 2026-09-17).** The user reversed the September 11 restriction. Public release builds and checks may use GitHub-hosted runners. Preserve all native package, privacy and upgrade gates. Review the verified artifacts before publishing a draft; do not substitute a workflow start for successful acceptance. Keep the private development repository's Actions disabled unless needed and separately authorized.
